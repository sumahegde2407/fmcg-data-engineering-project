...
df_silver = df_bronze.withColumn(
    "month",
    F.coalesce(
        F.to_date(F.col("month"), "yyyy/MM/dd"),
        F.to_date(F.col("month"), "dd/MM/yyyy"),
        F.to_date(F.col("month"), "yyyy-MM-dd"),
        F.to_date(F.col("month"), "dd-MM-yyyy")
    )
)
...
df_gold_price = spark.table("databricksmaster.gold.sb_dim_gross_price")
...
delta_table = DeltaTable.forName(spark, "databricksmaster.gold.dim_gross_price")
