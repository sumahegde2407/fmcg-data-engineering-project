...
df_silver = df_silver.select("product_code", "division", "category", "product", "variant", "product_id", "read_timestamp", "file_name")
...
delta_table = DeltaTable.forName(spark, "databricksmaster.gold.dim_products")
df_child_products = spark.sql(f"SELECT product_code, division, category, product, variant FROM databricksmaster.gold.sb_dim_products;")
...
# MAGIC %sql
# MAGIC select * from gold.sb_dim_products